...
function postMessage(req, res) {
  ...
  return dbGetUser(pool, userId).then((user) => {
    ...
    return dbAddMessage(pool, channel_id, userId, message).then(() =>
      res.status(204).end("")
    );
  });
}
...
function formatDate(dateStr) {
  ...
  const datePart = [
    d.getFullYear(),
    zeroPadd(d.getMonth() + 1, 2),
    zeroPadd(d.getDate(), 2),
  ].join("/");
  const timePart = [
    zeroPadd(d.getHours(), 2),
    zeroPadd(d.getMinutes(), 2),
    zeroPadd(d.getSeconds(), 2),
  ].join(":");
...
async function getMessage(req, res) {
  ...
  const rows = await pool.query(
    `SELECT
        m.id, m.created_at, m.content, u.name, u.display_name, u.avatar_icon
    FROM
        message m
  ...
  response.reverse();
  const maxMessageId = rows.length !== 0 ? rows[0].id : 0;
...
function postProfile(req, res) {
  ...
  return dbGetUser(pool, userId).then((user) => {
    ...
    if (avatarName && avatarData) {
      ...
      p = p.then(() =>
        pool.query("UPDATE user SET avatar_icon = ? WHERE id = ?", [
          avatarName,
          userId,
        ])
      );
    ...
    if (display_name) {
      p = p.then(() =>
        pool.query("UPDATE user SET display_name = ? WHERE id = ?", [
          display_name,
          userId,
        ])
      );
...
function getIcon(req, res) {
  ...
  return pool
    .query("SELECT data FROM image WHERE name = ? limit 1", [fileName])
    .then(([row]) => {
